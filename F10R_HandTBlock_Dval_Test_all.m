...
Xf1=hadamard(32); % 12,16,20,24,32,40
rr=2:m+1;
[rhad,chad]=size(Xf1);
...
XT13=[XT12c, zeros(12,1);zeros(1,12),xa(ja)]; % T7=T6+a
XT14=[XT8a,zeros(8,6);zeros(6,8),XT6b];
...
XT21=[XT20, zeros(20,1);zeros(1,20),xa(ja)]; % T7=T6+a
XT22= [XT20,zeros(20,2);zeros(2,20),X2(xb(jb),x12(j12))]; % XT6d+T2
XT23=[XT22, zeros(22,1);zeros(1,22),xa(ja)]; % T7=T6+a

    XT=XT10; % pick the values, XT6a XT6b XT6c XT6d
...
%fprintf('%s\n', text_to_print) % hide
P=fprintf(' %d,  %d, %d, %s, %.10f, %d', m, k, rhad, XTod, Done);
