...
Xf1=hadamard(20); % 8,12,16,20,24,32,40
rr=2:m+1;
[rhad,chad]=size(Xf1);
...
for jb=1:length(xb)
  ...
  XT4=[X4(x9(j9),x10(j10),x11(j11),x12(j12))];
...
XT7a=[XT6e, zeros(6,1);zeros(1,6),x3(j3)]; % T7=T6+a
XT7b=[XT3,zeros(3,4);zeros(4,3),XT4]; %T7=T3+T4
...
%fprintf('%s\n', text_to_print) % hide
fprintf(' %d,  %d, %d,  %s, %.10f, %d', m, k, rhad, XTod, Done);
