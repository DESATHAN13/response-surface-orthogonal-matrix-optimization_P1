...
Xf1=hadamard(12); % 8,12,16,20,24,32,40
rr=2:m+1;
[rhad,chad]=size(Xf1);
CC=nchoosek([2:rhad],5);
[ncc,mcc]=size(CC);
for ijk=1:ncc
Xf2=Xf1(:,CC(ijk,:));
...
%XT14=[XT8a,zeros(8,6);zeros(6,8),XT6d];
...
%XT20b= [XT10,zeros(10,10);zeros(10,10),XT10]; %T7=T3+T4

    XT=XT8a; % pick the values, XT6a XT6b XT6c XT6d
...
%fprintf('%s\n', text_to_print) % hide
P=fprintf(' %d,  %d, %d, "C5(6-1)", %s, %.10f, %d', m, k, rhad, XTod, Done);
